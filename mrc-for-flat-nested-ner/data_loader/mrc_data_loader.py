...
class MRCNERDataLoader(object):
    ...
    # def convert_examples_to_features(self, file, data_sign="train"):
        ...
        if data_sign == "train":
            ...
            # examples = self.data_processor.get_train_examples(file, self.data_dir)
            self.num_train_instances = len(examples)
        elif data_sign == "dev":
            examples = self.data_processor.get_dev_examples(self.data_dir)
            # examples = self.data_processor.get_dev_examples("mrc-ner.dev", self.data_dir)
            self.num_dev_instances = len(examples)
        elif data_sign == "test":
            examples = self.data_processor.get_test_examples(self.data_dir)
            # examples = self.data_processor.get_test_examples("mrc-ner.test", self.data_dir)
    ...
    # def get_dataloader(self, file, data_sign="train"):
        
        features = self.convert_examples_to_features(data_sign=data_sign)
        # features = self.convert_examples_to_features(file, data_sign=data_sign)
        print(len(features), data_sign)
        # print("data loaded")
        ...
        if data_sign == "train":
            ...
            dataloader = DataLoader(dataset, sampler=datasampler, batch_size=self.train_batch_size, num_workers=16, pin_memory=True)
        elif data_sign == "dev":
            datasampler = SequentialSampler(dataset)
            dataloader = DataLoader(dataset, sampler=datasampler, batch_size=self.dev_batch_size, num_workers=16, pin_memory=True)
        elif data_sign == "test":
            datasampler = SequentialSampler(dataset) 
            dataloader = DataLoader(dataset, sampler=datasampler, batch_size=self.test_batch_size, num_workers=16, pin_memory=True)
