# -*- coding: utf-8 -*-
"""Copy of train_bert_mrc.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1_eGLRyQCvguhsWi-erzlhnDdOp5RxyKy
# """
...
# def load_data(config, file):
    ...
    # train_dataloader = dataset_loaders.get_dataloader(file, data_sign="train") 
    # dev_dataloader = dataset_loaders.get_dataloader("mrc_ner.dev", data_sign="dev")
    # test_dataloader = dataset_loaders.get_dataloader("mrc_ner.test", data_sign="test")
...
# def train(model, optimizer, sheduler,  train_dataloader_list, dev_dataloader, test_dataloader, config, \
#     device, n_gpu, label_list):
    nb_tr_steps = 0 
    tr_loss = 0 
...
def eval_checkpoint(model_object, eval_dataloader, config, \
    ...
    for input_ids, input_mask, segment_ids, start_pos, end_pos, span_pos, ner_cate in eval_dataloader:
        ...
        # print(start_logits, start_label, end_label, span_label)
        ...
        # print(start_pred_lst, end_pred_lst, span_pred_lst)
...
# train_files = glob.glob(os.path.join(args_config.data_dir, "mrc-ner.train*"))
# change
torch.cuda.device_count()
train_loader, dev_loader, test_loader, num_train_steps, label_list = load_data(config)
# train_loader_list = []
# for file in train_files:
#     train_loader, dev_loader, test_loader, num_train_steps, label_list = load_data(config, file)
...
# train(model, optimizer, sheduler, train_loader_list, dev_loader, test_loader, config, device, n_gpu, label_list)

##args_config = args_parser()
#config = merge_config(args_config)

#model, optimizer, sheduler, device, n_gpu = load_model(config, 43386, ["GPE", "ORG", "PER", "FAC", "VEH", "LOC", "WEA", "O"])
